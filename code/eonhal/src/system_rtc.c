...
// RTC EXTI Lines
#define RTC_EXTI_LINE_WAKEUPTIMER LL_EXTI_LINE_20
#define RTC_EXTI_LINE_ALARM LL_EXTI_LINE_17
#define RTC_EXTI_LINE_TAMPER LL_EXTI_LINE_19
...
__STATIC_INLINE void RTC_setWAKEUPTIMER(uint32_t wakeupCounter, uint32_t wakeupClock)
{
	...
	LL_EXTI_EnableIT_0_31(RTC_EXTI_LINE_WAKEUPTIMER); //ok

	LL_EXTI_EnableRisingTrig_0_31(RTC_EXTI_LINE_WAKEUPTIMER); //ok
...
void rtc_setWKUPMillis(uint16_t milliseconds)
{
	...
	LL_EXTI_ClearFlag_0_31(RTC_EXTI_LINE_WAKEUPTIMER); //ok
	...
	LL_EXTI_DisableIT_0_31(RTC_EXTI_LINE_WAKEUPTIMER);

	if (milliseconds != 0)
	{
		NVIC_EnableIRQ(RTC_IRQn);
		ms_fix = (uint16_t)(((float)2.048 * milliseconds) - 1);
...
void rtc_setAlarmA(uint8_t RTC_ALARM_TYPE_X, uint8_t day_weekday, uint8_t hours, uint8_t minutes, uint8_t seconds)
{
	...
	LL_EXTI_EnableRisingTrig_0_31(RTC_EXTI_LINE_ALARM);
	LL_RTC_EnableWriteProtection(RTC);
}
...
{
	LL_RTC_BAK_SetRegister(RTC, BKP_DRx, (uint32_t)data);
}

uint32_t system_BKPRead(uint32_t BKP_DRx)
{
	return (uint32_t)LL_RTC_BAK_GetRegister(RTC, BKP_DRx);
}
...
void RTC_IRQHandler(void)
{
	...
	LL_EXTI_ClearFlag_0_31(RTC_EXTI_LINE_WAKEUPTIMER);
	LL_EXTI_ClearFlag_0_31(RTC_EXTI_LINE_ALARM);
	LL_EXTI_ClearFlag_0_31(RTC_EXTI_LINE_TAMPER);
}
